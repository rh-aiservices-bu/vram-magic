 ...
 */
export function generateBellCurveDistribution(
  numRequests: number,
 ...
 */
export function calculateConcurrentUsersVLLM(
  currentTime: number,
...
export function generateRequests(config: RequestConfig): RequestObject[] {
  ...
  if (distribution === 'bell_curve') {
    ...
    timestamps = Array.from({ length: totalRequests }, (_, i) => (i / totalRequests) * duration)
  } else {
...
export function runVLLMSimulation(config: SimulationConfig): SimulationResult {
  ...
  for (let t = 0; t <= duration; t += sampleInterval) {
    ...
    if (adjustedConcurrentUsers > 0) {
      ...
      const baseVRAM =
        model.parameters * (PRECISION_BYTES[precision as keyof typeof PRECISION_BYTES] || 2) * 1.2
      samples.push({
        timestamp: t,
        concurrentUsers: 0,
