...
export function calculateActivationsVLLM(
  ...
  const activationBytes =
    arch.hiddenSize * sequenceLength * batchSize * precisionBytes * activationMultiplier // 1.5x for inference, not 4x for training!
...
export function calculateTotalVRAMvLLM(
  ...
  const precisionBytes =
    (PRECISION_BYTES && PRECISION_BYTES[precision as keyof typeof PRECISION_BYTES]) ||
    FALLBACK_PRECISION_BYTES[precision] ||
    2

  // 1. Base model memory (weights)
  const baseModelBytes =
    model.parameters * precisionBytes * (model.vramRequirements?.overheadFactor || 1.2)
  ...
  const kvCache = calculateKVCacheVLLM(model, sequenceLength, concurrentUsers, precisionBytes)
...
export function compareCalculationMethods(
  ...
  const oldActivations =
    model.architecture.hiddenSize * sequenceLength * concurrentUsers * precisionBytes * 4 // Training multiplier!
